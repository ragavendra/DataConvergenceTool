...
$sMoveInDate=$_POST['MoveInDate'];
$sMoveInDateTo=$_POST['MoveInDateTo'];
...
session_start();
if (($_POST['MDMS'] == "MDMS") AND ($_SESSION['$sMDMS'] == "No"))
{
$_SESSION['$sMDMS']=$_POST['MDMS'];
}
elseif ((!isset($_SESSION['$sMDMS'])) AND ($_POST['MDMS'] == ""))
{
$_SESSION['$sMDMS'] = "No";
}

if (($_POST['ADCS'] == "ADCS") AND ($_SESSION['$sADCS'] == "No"))
{
$_SESSION['$sADCS']=$_POST['ADCS'];
}
elseif ((!isset($_SESSION['$sADCS'])) AND ($_POST['ADCS'] == ""))
{
$_SESSION['$sADCS'] = "No";
}
	...
	if ($sBillingClass!="")
{
	...
	if ($sMoveInDate!="")
{
		//$sDMTQuery = $sDMTQuery . " AND MoveInDate BETWEEN STR_TO_DATE( `$sMoveInDate` , '%Y-%m-%d' ) AND STR_TO_DATE( '$sMoveInDateTo', '%Y-%m-%d' )";
		$sDMTQuery = $sDMTQuery . "AND STR_TO_DATE( `MoveInDate` , '%d.%m.%Y' ) > STR_TO_DATE( '$sMoveInDate', '%Y-%m-%d' )";
		$sNBFlag = False;
}
	if ($sMoveInDateTo!="")
{
		//$sDMTQuery = $sDMTQuery . " AND MoveInDate BETWEEN STR_TO_DATE( `$sMoveInDate` , '%Y-%m-%d' ) AND STR_TO_DATE( '$sMoveInDateTo', '%Y-%m-%d' )";
		$sDMTQuery = $sDMTQuery . "AND STR_TO_DATE( `MoveInDate` , '%d.%m.%Y' ) < STR_TO_DATE( '$sMoveInDateTo', '%Y-%m-%d' )";
		$sNBFlag = False;
}
...
$sDMTQueryCount = "SELECT COUNT(DISTINCT SerialNumber) as TotalRows FROM $sTableName WHERE SystemStatus = 'INST'" . $sDMTQuery;
